...
const Publications = async () => {
  ...
  return (
    <>
      <Header />
      <h1 className="flex justify-center font-bold text-[3em] mt-[1em]">
        Публікації
      </h1>
          ...
          <nav>
            <ul className="flex gap-2">
              {categoty.map((item, index) => (
                <li
                  key={index}
                  className="flex items-center font-bold py-3 px-5 rounded-xl bg-[--mainColor] transform transition-all duration-300 hover:text-white hover:bg-[--buttonColor]"
                >
                  <a href="">{item.name}</a>
                </li>
              ))}
